...
import prednet_segmentation
from prednet_segmentation.ConvLSTMCell import ConvLSTMCell
from prednet_segmentation.hConvGRUCell import hConvGRUCell
import signal
import sys
import time
...
class PredNet(nn.Module):
    ...
    def __init__(self,
                 model_directory, model_name, device,
        ...
        self.device = device
        p = f'{model_directory}/{model_name}/'
...
class PrednetSegmentation(StepModule):
    def __init__(self, name, gpu_factor, device, batch_size, n_channels, im_w, im_h, n_frames):
        super().__init__(name)
        ...
        self.prednet = PredNet(
            model_directory=prednet_segmentation.ts_model_dir,
            model_name='prednet',
            device=self.device,
            A_channels=(self.n_channels, 16, 64),
    ...
    def ExecuteStep(self, time):
        #self.E_seq, self.R_seq, self.P_seq, self.S_seq = model(input_.to(device))
        print("Prednet Module Executing")

        input_ = torch.rand(self.batch_size, self.n_channels, self.im_w, self.im_h, self.n_frames)
        E_seq, R_seq, P_seq, S_seq = self.prednet(input_.to(self.device))
