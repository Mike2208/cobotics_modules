...
from osim_python.ybot_hand_geometry import right_hand_links, pose_rotation_matrix, right_hand_center

r_hand_mass_center = np.array([-0.0030131400000000001, -0.042499299999999997, -0.0011220500000000001])
...
class OsimModel(object):
    def __init__(self, model_path, step_size, integrator_accuracy, body_ext_force, visualize):
        ...
        if body_ext_force is not None:
            for i, (link, loc) in enumerate(right_hand_links.items()):
                osim_pos = pose_rotation_matrix @ (loc - right_hand_center)
                prescribed_force = opensim.PrescribedForce(link, self.body_set.get(body_ext_force))
                prescribed_force.setPointFunctions(opensim.Constant(osim_pos[0]), opensim.Constant(osim_pos[1]),
                                                   opensim.Constant(osim_pos[2]))
    ...
    def get_state_dict(self):
        ...
        for i in range(self.coordinate_set.getSize()):
            coordinate = self.coordinate_set.get(i)
            name = coordinate.getName()
            obs['coordinate_muscle_moment_arm'][name] = {}
            if self.model.getMuscles().getSize() != 0:
                for i in range(self.model.getMuscles().getSize()):
                    muscle = self.model.getMuscles().get(i)
                    ...
                    obs['coordinate_muscle_moment_arm'][name][name_muscle] = muscle_.computeMomentArm(self.state, coord)
    ...
    def reset(self):
        ...
        self.step_alive = 0
        self.reset_manager()
        self.kinematics.getPositionStorage().reset(0)
        self.kinematics.getVelocityStorage().reset(0)
        self.kinematics.getAccelerationStorage().reset(0)
    ...
    def save_simulation(self, base_dir):
        ...
        self.kinematics.printResults('simulation', base_dir)
        self.body_kinematics.printResults('body_simulation', base_dir)
        print(self.body_kinematics.getPositionStorage())
