...
ROSTopicOutputDriver::ROSTopicOutputDriver(string TopicName, unsigned int MaxSpikeBuffered, double InitSimulationRosTime, double output_delay):
	...
	this->Publisher_group = this->NodeHandler.advertise<edlut_ros_osim::Spike_group>(TopicName, MaxSpikeBuffered);
...
void ROSTopicOutputDriver::WriteSpike(const Spike * NewSpike){
	if(index < 1000){
...
void ROSTopicOutputDriver::WriteState(float Time, Neuron * Source){
	return;
}
...
void ROSTopicOutputDriver::FlushBuffers(){
