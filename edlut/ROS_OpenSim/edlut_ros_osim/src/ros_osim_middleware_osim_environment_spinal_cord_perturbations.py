...
import random
import spinal_cord.sc_circuits
...
class ROS_OpenSim_middleware():
    def __init__(self, joint_list, muscle_list, joint_position_topic, joint_velocity_topic, joint_acceleration_topic,
        ...
        self.hand_contact_sensor_subscriber = rospy.Subscriber(hand_contact_sensor_topic, ContactsState, self.contactSensorCallback)
    ...
    def getSensorCallbackFunctions(self, links):
        pass
    # Callback for the contact sensor signal.
    def contactSensorCallback(self, contactsensor_msg):
        if len(contactsensor_msg.states) > 0:
            ...
            self.hand_total_force = np.array([total_force_x, total_force_y, total_force_z])
        else:
            self.hand_total_force = np.zeros(3)

        self.force_time_stamp = int(str(contactsensor_msg.header.stamp))
...
def main():
    ...
    while not rospy.is_shutdown():
        ...
        if use_sim_time:
            ...
            if new_time > current_time:
                if rosOsim.inputUpdate:
                    ...
                    final_excitation = spinal_cord.sc_circuits.muscle_excit(sc_model.mn_rates)
        ...
        # If real time: run the simulation
            ...
            perturbation_value = 0.0
            if perturbation_index < len(perturbation_force):
                if current_time>=perturbation_init_time[perturbation_index] and current_time <= perturbation_end_time[perturbation_index]:
                    ext_force = opensim.PrescribedForce.safeDownCast(model.model.getForceSet().get('perturbation'))
                    forceFunctionSet = ext_force.get_forceFunctions()
                    func = opensim.Constant.safeDownCast(forceFunctionSet.get(0)) #0 for Fx, 1 for Fy
                    perturbation_value = perturbation_force[perturbation_index]
