...
#include "edlut_ros_osim/ROSTopicInputDriver.h"
#include "edlut_ros_osim/ROSTopicOutputDriver.h"
...
#include "edlut_ros_osim/TrafficDelay.h"
...
#pragma omp parallel if(!use_sim_time) num_threads(2)
{
		...
		if(omp_get_thread_num()==1){
			...
			while (!stop_simulation){
				...
				if (time>=last_print_time+ros::Duration(1.0)){
					ROS_INFO("EDLUT Simulator: Simulating until time %f with thread %d", time.toSec(), omp_get_thread_num());
					last_print_time = time;
				}
