...
#include <edlut_ros_osim/Analog.h>
...
int main(int argc, char **argv)
{
	...
	std::vector<std::string> joint_list, muscle_list;
	std::vector<double> phase, amplitude, frequency;
	std::string control_topic, joint_command_topic, joint_state_topic, desired_position_topic, desired_velocity_topic, positions_file_name, velocities_file_name, muscle_activation_file_name, muscle_activation_topic;
	...
	private_node_handle_.getParam("muscle_activation_file_name", muscle_activation_file_name);
	private_node_handle_.getParam("muscle_list", muscle_list);
	...
	private_node_handle_.getParam("muscle_activation_topic", muscle_activation_topic);
	...
	Q_TrajectoryGenerator_andMuscleActivation TrajGenerator (use_sim_time, max_pos_amplitude, min_pos_amplitude, max_vel_amplitude,
		min_vel_amplitude, samples, positions_file_name, velocities_file_name, trajectory_frequency, joint_list, muscle_activation_file_name, muscle_list);

	// Create the trajectory controller
	Q_TrajectoryController_compactDelay_andMuscleActivation TrajController(joint_list,
			muscle_list,
			desired_position_topic,
			desired_velocity_topic,
			muscle_activation_topic,
			control_topic,
			// joint_command_topic,
			//joint_state_topic,
			&TrajGenerator,
