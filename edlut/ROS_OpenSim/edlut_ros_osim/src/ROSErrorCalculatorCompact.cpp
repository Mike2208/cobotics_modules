...
#include "edlut_ros_osim/ROSErrorCalculatorCompact.h"
#include "edlut_ros_osim/AnalogCompact.h"

void ROSErrorCalculatorCompact::PositionPlusCallback(const edlut_ros_osim::AnalogCompact::ConstPtr& msg){
	...
	if (msg->header.stamp.toSec()<this->last_time_pos_plus){
		...
		edlut_ros_osim::AnalogCompact newMessage;
...
void ROSErrorCalculatorCompact::PositionMinusCallback(const edlut_ros_osim::AnalogCompact::ConstPtr& msg){
	// Check if the spike should have been processed previously
...
void ROSErrorCalculatorCompact::VelocityPlusCallback(const edlut_ros_osim::AnalogCompact::ConstPtr& msg){
	// Check if the spike should have been processed previously
...
void ROSErrorCalculatorCompact::VelocityMinusCallback(const edlut_ros_osim::AnalogCompact::ConstPtr& msg){
	// Check if the spike should have been processed previously
				...
				last_time_vel_minus(0.0)
{
	...
	this->publisher = this->NodeHandler.advertise<edlut_ros_osim::AnalogCompact>(output_topic_name, 1.0);
}
...
void ROSErrorCalculatorCompact::UpdateError(ros::Time current_time){
	...
	edlut_ros_osim::AnalogCompact newMsg;
...
void ROSErrorCalculatorCompact::CleanQueue(std::priority_queue<edlut_ros_osim::AnalogCompact, std::vector<edlut_ros_osim::AnalogCompact>, analog_comparison > & queue,
		std::vector<double> & updateVar,
		double & lastTime,
		double end_time){
	edlut_ros_osim::AnalogCompact top_value;
