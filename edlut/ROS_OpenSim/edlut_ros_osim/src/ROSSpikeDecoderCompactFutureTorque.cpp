...
#include "edlut_ros_osim/ROSSpikeDecoderCompactFutureTorque.h"
#include "edlut_ros_osim/Spike.h"
#include "edlut_ros_osim/Spike_group.h"

void ROSSpikeDecoderCompactFutureTorque::SpikeCallback(const edlut_ros_osim::Spike_group::ConstPtr& msg){
...
std::vector<edlut_ros_osim::AnalogCompact> ROSSpikeDecoderCompactFutureTorque::UpdateDecoder(double end_time){
	...
	if (!this->activity_queue.empty()){
		...
		while (!(this->activity_queue.empty()) &&	top_spike.time<=checkpoint){
			...
			if (top_spike.time > checkpoint){
				edlut_ros_osim::AnalogCompact newTorque;
