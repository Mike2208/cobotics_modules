...
#include "edlut_ros_osim/ROSFullStateCalculatorCompact.h"
#include "edlut_ros_osim/AnalogCompact.h"
#include "edlut_ros_osim/FullStateCompactDelay.h"


void ROSFullStateCalculatorCompact::DesiredPositionCallback(const edlut_ros_osim::AnalogCompactDelay::ConstPtr& msg){
	...
	if (msg->header.stamp.toSec()<this->last_time_desired_pos){
		...
		edlut_ros_osim::AnalogCompactDelay newMessage;
...
void ROSFullStateCalculatorCompact::CurrentPositionCallback(const edlut_ros_osim::AnalogCompactDelay::ConstPtr& msg){
	// Check if the spike should have been processed previously
...
void ROSFullStateCalculatorCompact::DesiredVelocityCallback(const edlut_ros_osim::AnalogCompactDelay::ConstPtr& msg){
	// Check if the spike should have been processed previously
...
void ROSFullStateCalculatorCompact::CurrentVelocityCallback(const edlut_ros_osim::AnalogCompactDelay::ConstPtr& msg){
	// Check if the spike should have been processed previously
				...
				sampling_frequency(sampling_frequency)
{
	...
	this->publisher = this->NodeHandler.advertise<edlut_ros_osim::FullStateCompactDelay>(output_topic_name, 1.0);
...
void ROSFullStateCalculatorCompact::UpdateError(ros::Time current_time){
	...
	edlut_ros_osim::FullStateCompactDelay newMsg;
...
void ROSFullStateCalculatorCompact::CleanQueue(std::priority_queue<edlut_ros_osim::AnalogCompactDelay, std::vector<edlut_ros_osim::AnalogCompactDelay>, analog_comparison > & queue,
		std::vector<double> & updateVar,
		double & updateDelay,
		double compensateDelay,
		double & lastTime,
		double end_time){
	edlut_ros_osim::AnalogCompactDelay top_value;
