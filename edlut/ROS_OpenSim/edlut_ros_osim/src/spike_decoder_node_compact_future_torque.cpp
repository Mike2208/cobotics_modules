...
#include <edlut_ros_osim/Time.h>
#include <edlut_ros_osim/ROSSpikeDecoderCompactFutureTorque_compactDelay.h>
#include <ros/ros.h>
...
public:
	...
	void DelayCallback(const edlut_ros_osim::Time::ConstPtr& msg){
		this->delay = msg->data;
		return;
	}
...
private:
	...
	std::vector<edlut_ros_osim::AnalogCompactDelay> torque_queue;
...
public:
	...
	void InsertandOrderBuffers(edlut_ros_osim::AnalogCompactDelay newMessage, double newDelay){
		this->torque_queue.insert(this->torque_queue.begin(), newMessage);
		this->delay_queue.insert(this->delay_queue.begin(), newDelay);

		edlut_ros_osim::AnalogCompactDelay aux_torqueMsg;
		double aux_delay;
	...
	edlut_ros_osim::AnalogCompactDelay GetTopValue_Torque(){
...
int main(int argc, char **argv)
{
	...
	std::vector<edlut_ros_osim::AnalogCompactDelay> torque_buffer;
  ...
  nh.setCallbackQueue(&CallbackQueue);
	...
	ros::Publisher output_publisher = nh.advertise<edlut_ros_osim::AnalogCompactDelay>(output_topic, 1.0);
	...
	while (!stop_node){
		...
		edlut_ros_osim::AnalogCompactDelay msg;
...
//end new buffer delays manager
    ...
    // 	std::copy(analog_value.begin(), analog_value.end(), std::ostream_iterator<double>(oss, ","));
		...
		//
		// edlut_ros_osim::AnalogCompact msg;
