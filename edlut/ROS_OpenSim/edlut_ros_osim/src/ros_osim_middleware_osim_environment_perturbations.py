...
def main():
    ...
    # Initial time of each of the motor pertubations
    perturbation_init_time = rospy.get_param("~perturbation_init_time")
    # End time of each of the motor pertubations
    perturbation_end_time = rospy.get_param("~perturbation_end_time")
    # Force in N of each of the motor pertubations
    perturbation_force = rospy.get_param("~perturbation_force")
    # Body suffering the motor pertubations
    perturbation_body = rospy.get_param("~perturbation_body")
    # Topic to publish the motor pertubations
    perturbation_topic = rospy.get_param("~perturbation_topic")

    # Index pointing to the perturbation to be applied next
    ...
    # Load OpenSim model
    model = OsimModel(osim_model, step_size, integrator_accuracy, perturbation_body[0], visualize=osim_visualization)
        ...
        if use_sim_time:
            ...
            if new_time > current_time:
                if rosOsim.inputUpdate:
                    ...
                    # add motor perturbations to the model
                    perturbation_value = 0.0
                    if perturbation_index < len(perturbation_force):
                        if current_time.to_sec()>=perturbation_init_time[perturbation_index] and current_time.to_sec() <= perturbation_end_time[perturbation_index]:
                            ...
                            func = opensim.Constant.safeDownCast(forceFunctionSet.get(0)) #0 for Fx, 1 for Fy
                            perturbation_value = perturbation_force[perturbation_index]
                            func.setValue(perturbation_value) # in N
                        elif current_time.to_sec()>perturbation_end_time[perturbation_index]:
                    ...
                    # Publish the motor perturbation value
                    for x in range(0, len(perturbation_body)):
                        perturbation_msg.data[x] = perturbation_value
                    perturbation_msg.header.stamp = current_time
                    perturbation_publisher.publish(perturbation_msg)
