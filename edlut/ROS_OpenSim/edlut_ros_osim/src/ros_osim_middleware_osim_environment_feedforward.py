...
class ROS_OpenSim_middleware():
    def __init__(self, joint_list, muscle_list, joint_position_topic, joint_velocity_topic,
                current_muscle_activation_topic, input_muscle_activation_topic, desired_torque_topic,
                computed_muscle_activation_topic, total_muscle_activation_topic, activation_exponent, use_sim_time, step_size, marker_list, marker_kinematics_topic):
        ...
        # (total = muscle activation received from other module (if any) plus the activation computed from desired torque)
        self.total_muscle_activation = []

        # Excitation obtained from the computed or received muscle activations
        self.computed_excitation = []
        self.received_excitation = []
        self.final_excitation = []
        ...
        self.marker_position = len(self.marker_list)*[[0, 0, 0]] # position = [[marker1_pos_x, marker1_pos_y, marker1_pos_z], [marker2_pos_x, marker2_pos_y, marker2_pos_z]...]
        self.marker_velocity = len(self.marker_list)*[[0, 0, 0]]
        self.marker_acceleration = len(self.marker_list)*[[0, 0, 0]]
        self.cartesian_coordinates = ['x', 'y', 'z']
        ...
        for muscle in range(0, len(self.muscle_list)):
            ...
            self.total_muscle_activation.append(0.0)
            self.computed_excitation.append(0.0)
            self.received_excitation.append(0.0)
            self.final_excitation.append(0.0)
        ...
        self.computed_muscle_activation_publisher = rospy.Publisher(computed_muscle_activation_topic, AnalogCompactDelay, queue_size = 10)
        self.total_muscle_activation_publisher = rospy.Publisher(total_muscle_activation_topic, AnalogCompactDelay, queue_size = 10)
        ...
        self.desired_torque_subscriber = rospy.Subscriber(desired_torque_topic, AnalogCompactDelay, self.desiredTorqueCallback)
        ...
        self.total_muscle_activation_msg = AnalogCompactDelay()
        self.total_muscle_activation_msg.names = self.muscle_list
        self.total_muscle_activation_msg.data = [0] * len(self.muscle_list)
        ...
        self.marker_kinematics_msg.position = len(self.marker_list)*[[0, 0, 0]]
        self.marker_kinematics_msg.velocity = len(self.marker_list)*[[0, 0, 0]]
        self.marker_kinematics_msg.acceleration = len(self.marker_list)*[[0, 0, 0]]
    ...
    def getModelState(self, state, current_time):
        ...
        for index in range(0, len(self.marker_list)):
            if self.marker_list[index] in state['markers']:
                for i in range(0, len(self.cartesian_coordinates)):
                    self.marker_position[index][i] = state['markers'][self.marker_list]['pos'][i]
                    self.marker_velocity[index][i] = state['markers'][self.marker_list]['vel'][i]
                    self.marker_acceleration[index][i] = state['markers'][self.marker_list]['acc'][i]
        ...
        #     if state['muscles'][muscle]['force_length'] < 0:
        #         print "NEGATIVE FORCE LENGTH, muscle: ", muscle
        #     if state['muscles'][muscle]['force_velocity'] < 0:
        #         print "NEGATIVE FORCE VELOCITY, muscle: ", muscle
        #     if state['muscles'][muscle]['passive_force'] < 0:
        #         print "NEGATIVE PASSIVE FORCE, muscle: ", muscle
        #
        # for joint in self.joint_list:
        #     for muscle in self.muscle_list:
        #         if state["coordinate_muscle_moment_arm"][joint][muscle] < 0:
        #             print "NEGATIVE R, joint: ", joint, "muscle: ", muscle
    ...
    def publishMarkerKinematics(self, time_stamp):
        # Update the message data reading from muscle_activation
        for marker in range(0, len(self.marker_kinematics_msg.position)):
            for i in range(0, len(self.cartesian_coordinates)):
                self.marker_kinematics_msg.position[marker][i] = self.marker_position[marker][i]
                self.marker_kinematics_msg.velocity[marker][i] = self.marker_velocity[marker][i]
                self.marker_kinematics_msg.acceleration[marker][i] = self.marker_acceleration[marker][i]
        # Set the message time stamp
    ...
    def desiredTorqueCallback(self, received_torque_msg):
        ...
        self.totalActivationToExcitation()
    ...
    def torqueToActivation_oneJoint(self, current_time):
        ...
        # add the received pre-computed activation (obtained using Static Optimization) to the activation obtained from torque
        for i in range(0, len(self.computed_muscle_activation)):
            self.total_muscle_activation[i] = self.computed_muscle_activation[i] + self.received_muscle_activation[i]
            self.total_muscle_activation_msg.data[i] = self.total_muscle_activation[i]
        self.total_muscle_activation_msg.header.stamp = current_time
        self.total_muscle_activation_publisher.publish(self.total_muscle_activation_msg)
    ...
    def totalActivationToExcitation(self):
        self.final_excitation = self.activationToExcitation(self.total_muscle_activation, self.previous_computed_muscle_activation)
    ...
    def getFinalExcitation(self):
        return self.final_excitation
...
def main():
    ...
    # Topic to publish the muscle activation computed from torque
    computed_muscle_activation_topic = rospy.get_param("~computed_muscle_activation_topic")

    # Topic to publish the muscle activation computed from torque plus the received muscle activation from Static Optimization
    total_muscle_activation_topic = rospy.get_param("~total_muscle_activation_topic")

    # Received torque to be transformed into muscles activation to be applied to the OpenSim model
    desired_torque_topic = rospy.get_param("~desired_torque_topic")
    ...
    rosOsim = ROS_OpenSim_middleware(joint_list, muscle_list, joint_position_topic, joint_velocity_topic,
                                    current_muscle_activation_topic, input_muscle_activation_topic, desired_torque_topic,
                                    computed_muscle_activation_topic, total_muscle_activation_topic, activation_exponent, use_sim_time, step_size, marker_list, marker_kinematics_topic)
    ...
    while not rospy.is_shutdown():
        ...
        if use_sim_time:
            ...
            if new_time > current_time:
                if rosOsim.inputUpdate:
                    ...
                    excitation = rosOsim.getFinalExcitation()
                    nan = False
                    for x in excitation:
                        if math.isnan(x):
                            nan = True
                    if not nan:
                        final_excitation = excitation
        ...
        # If real time: run the simulation
            ...
            current_time = rospy.get_rostime()
            excitation = rosOsim.getComputedExcitation()
            nan = False
            for x in excitation:
                if math.isnan(x):
                    nan = True
