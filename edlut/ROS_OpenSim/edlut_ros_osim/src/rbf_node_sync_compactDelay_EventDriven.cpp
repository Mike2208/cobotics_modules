 ...
 ***************************************************************************/
 ...
 #include <edlut_ros_osim/ROSRBFBank_compactDelay.h>
 ...
 public:
    ...
    this->new_value = false;
 	...
 	void AnalogCallback(const edlut_ros_osim::AnalogCompactDelay::ConstPtr& msg){
    ...
    this->new_value = true;
 	...
 	void ControlCallback(const edlut_ros_osim::LearningState::ConstPtr& lear){
 ...
 int main(int argc, char **argv)
 {
  ...
  if (use_sim_time){
    ...
    // latch = True --> Publish the last message when a new subscriber is registered. Synchronize clock at the beginning of the simulation.
  ...
  // Create the output spike publisher
 	ros::Publisher output_publisher = nh.advertise<edlut_ros_osim::Spike>(output_topic+"OLD", 1e9);
  ros::Publisher output_publisher_group = nh.advertise<edlut_ros_osim::Spike_group>(output_topic, 1e9);
 	...
 	while (!stop_node){
        ...
        ros::Duration new_sampling_period = new_time - time;
				...
				if (objSensorState.IsInLearning()){
					ROS_DEBUG("RBF node: Generating activity at time %lf ", time.toSec());
          edlut_ros_osim::Spike_group spike_group;
