...
void ROSSpikeDecoderCompactFutureTorque_compactDelay::SpikeCallback(const edlut_ros_osim::Spike_group::ConstPtr& msg){
				...
				last_spike_time(0.0)
{
	...
	ros::SubscribeOptions subscriberOptions = ros::SubscribeOptions::create<edlut_ros_osim::Spike_group>(
...
std::vector<edlut_ros_osim::AnalogCompactDelay> ROSSpikeDecoderCompactFutureTorque_compactDelay::UpdateDecoder(double end_time){
	...
	edlut_ros_osim::Spike top_spike;
	...
	if (!this->activity_queue.empty()){
		...
		while (!(this->activity_queue.empty()) &&	top_spike.time<=checkpoint){
			...
			if (top_spike.time > checkpoint){
				edlut_ros_osim::AnalogCompactDelay newTorque;
