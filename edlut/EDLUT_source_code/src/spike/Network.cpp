...
NeuronModel ** Network::LoadNetTypes(string ident_type, string neutype, int & ni, long Currentline, const char * netfile){
	NeuronModel ** type;
...
Network::Network(const char * netfile, const char * wfile, EventQueue * Queue, int numberOfQueues) : inters(0), ninters(0), neutypes(0), nneutypes(0), neurons(0), nneurons(0), timedrivenneurons(0), ntimedrivenneurons(0), wchanges(0), nwchanges(0), wordination(0), NumberOfQueues(numberOfQueues), minpropagationdelay(0.0001), invminpropagationdelay(1.0 / 0.0001), monitore_neurons(false){
...
void Network::LoadNet(const char *netfile) {
	FILE *fh;
...
void Network::LoadWeights(const char *wfile){
	FILE *fh;
	int connind;
	long Currentline=1L;
	fh=fopen(wfile,"rt");
