...
class Network : public PrintableObject{
	private:
		 ...
		 * \param netfile network file name
   		 ...
   		 */
		NeuronModel ** LoadNetTypes(string ident_type, string neutype, int & ni, long Curentline, const char * netfile);
		...
		void InitializeStates(int ** N_neurons);
   		 ...
   		 */
   		void LoadNet(const char *netfile) ;
   		 ...
   		 */
   		void LoadWeights(const char *wfile);
		...
		*/
		bool monitore_neurons;
   		 ...
   		 */
   		Network(const char * netfile, const char * wfile, EventQueue * Queue, int numberOfQueues);
